        ...
        if ($po_id <= 0) {
            ...
            if (!$po) {
                ...
                if (!$totals['has_delivery']) {
                    $db->prepare("UPDATE purchase_orders
                        SET supplier_status='Pending Confirmation', updated_at=NOW()
                        WHERE po_id=?")
                       ->execute([$po_id]);
                }

                $msg = 'Purchase Order confirmed and sent to Supplier Portal.';
                $msgType = 'success';
            }
        }
    }
        ...
        <tr>
          <td>
            ...
            <?php if ((int)$po['is_draft'] === 1): ?>
              <br><span class="badge badge-warning">Pending Owner Confirmation</span>
            <?php endif; ?>
          </td>
