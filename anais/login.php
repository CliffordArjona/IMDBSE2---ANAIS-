    ...
    if ($username === '' || $password === '') {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if ($loginType === 'supplier' && $role !== 'Supplier') {
                ...
                // Track if the account is already being used.
                try {
                    $db->prepare("UPDATE users SET last_login_at = NOW() WHERE user_id = ?")
                       ->execute([(int)$user['user_id']]);
                } catch (Throwable $e) {
                    // Optional column for old databases; do not block login.
                }
...
<div class="login-page">
  <div class="login-card">
    ...
    <div id="staffForgotLink" style="text-align:center;margin-top:10px;font-size:13px;<?= $loginType === 'staff' ? '' : 'display:none' ?>">
      <a href="/anais/staff_forgot_password.php">Forgot temporary password?</a>
    </div>
...
document.querySelectorAll('.login-tab').forEach(function(btn) {
  btn.addEventListener('click', function() {
    ...
    const staffForgotLink = document.getElementById('staffForgotLink');
    if (staffForgotLink) {
      staffForgotLink.style.display = type === 'staff' ? '' : 'none';
    }
